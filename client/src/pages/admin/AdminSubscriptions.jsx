...
const StarIcon = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>);
...
function AdminSubscriptions() {
  ...
  const handleEditPlan = (plan) => {
    setPlanForm({
      plan_name: plan.plan_name, price: plan.price, product_limit: plan.product_limit,
      verified_seller_badge: plan.verified_seller_badge, verified_product_badge: plan.verified_product_badge,
      premium_badge: plan.premium_badge, shop_badge: plan.shop_badge,
      shop_enabled: plan.shop_enabled, vip_badge: plan.vip_badge,
    });
  ...
  const handleUpdatePlan = async (e) => {
    ...
    try {
      ...
      if (data.success) { 
        ...
        setTimeout(() => { setPlanFormSuccess(""); fetchSubscriptions(); }, 1500); 
      } else { setPlanFormError(data.message); }
    } catch (err) { setPlanFormError("Failed to update plan"); } finally { setPlanFormLoading(false); }
  };

  const features = [
    { key: 'verified_seller_badge', label: 'Verified Seller Badge' },
    { key: 'verified_product_badge', label: 'Verified Product Badge' },
  ...
  return (
    <div>
      <style>{`
        ...
        input:focus { border-color: #00E309 !important; outline: none; }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1.2rem' }}>
        {subscriptions.map((plan) => (
          <div key={plan.id} 
            className={`plan-card ${plan.vip_badge ? 'vip' : ''}`}
            style={{ 
              background: plan.vip_badge ? 'rgba(234,179,8,0.06)' : 'rgba(26,26,46,0.5)',
              backdropFilter: 'blur(16px)',
              borderRadius: 20, 
              padding: '1.6rem', 
              textAlign: 'center',
              border: plan.vip_badge ? '2px solid rgba(234,179,8,0.3)' : '1px solid rgba(255,255,255,0.08)',
              position: 'relative',
              cursor: 'default'
            }}>
            ...
            <h3 style={{ 
              ...
              color: plan.vip_badge ? '#eab308' : 'white',
              margin: `${plan.vip_badge ? '1.5rem 0 0.5rem' : '0 0 0.5rem'}`
            }}>
              {plan.plan_name}
            </h3>
            ...
            <div style={{ 
              ...
              <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#00E309' }}>
                {plan.product_limit === 999999 ? "Unlimited" : plan.product_limit} Products
              </span>
            </div>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.2rem', fontWeight: 700, color: 'white', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <EditIcon /> Edit {showEditPlan.plan_name} Plan
              </h2>
            ...
            <form onSubmit={handleUpdatePlan} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input type="text" value={planForm.plan_name} onChange={e => setPlanForm(p => ({...p, plan_name: e.target.value}))} placeholder="Enter plan name"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <input type="number" value={planForm.price} onChange={e => setPlanForm(p => ({...p, price: Number(e.target.value)}))} placeholder="0"
                    style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', fontSize: '0.85rem', color: 'white', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 600, marginBottom: '0.3rem', color: 'white', textTransform: 'uppercase', letterSpacing: '0.03em' }}>Product Limit</label>
                  <input type="number" value={planForm.product_limit} onChange={e => setPlanForm(p => ({...p, product_limit: Number(e.target.value)}))} placeholder="10"
                    style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', fontSize: '0.85rem', color: 'white', boxSizing: 'border-box' }} />
                </div>
              </div>
